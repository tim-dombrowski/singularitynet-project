...
Now let's conduct some more formal statistical analysis to explore the relationships between the asset returns. For brevity, the focus from this point on will exclusively focus on the excess returns for the Complete data series. Feel free to expand out to other versions of the final data to explore those results.
...
```{r capmregs}
AGIXfit = lm(AGIX~SPY, data=daily_annrets_xs_complete)
summary(AGIXfit)
SDAOfit = lm(SDAO~SPY, data=daily_annrets_xs_complete)
summary(SDAOfit)
NTXfit = lm(NTX~SPY, data=daily_annrets_xs_complete)
summary(NTXfit)
RJVfit = lm(RJV~SPY, data=daily_annrets_xs_complete)
summary(RJVfit)
HYPCfit = lm(HYPC~SPY, data=daily_annrets_xs_complete)
summary(HYPCfit)
CGVfit = lm(CGV~SPY, data=daily_annrets_xs_complete)
summary(CGVfit)
SOPHfit = lm(SOPH~SPY, data=daily_annrets_xs_complete)
summary(SOPHfit)
BTCfit = lm(BTC~SPY, data=daily_annrets_xs_complete)
summary(BTCfit)
ETHfit = lm(ETH~SPY, data=daily_annrets_xs_complete)
summary(ETHfit)
NVDAfit = lm(NVDA~SPY, data=daily_annrets_xs_complete)
summary(NVDAfit)
```

The key observation from these regression results is in the "Multiple R-squared" value, which is a measure of close of a relationship there is between the two variables. Of these assets, only NVDA exhibits a strong relationship with the market portfolio, which is not surprising given that it is the only stock in the group. The other crypto-assets all have low R-squared values, which is consistent with the generally low correlation between the crypto-assets and the stock market. This can also been seen in the model "p-value" estimates. Of which, some of the cryptos generate marginal significance. But overall, the CAPM model only appears appropriate for NVDA, and not the others.

The closeness of fit from these models is key to drawing any other conclusions from the model estimates. So while we could compare all the CAPM betas, only the NVDA beta has any real meaning since the others lack statistical power. Thus, the NVDA alpha and beta estimates are the only ones that can be interpreted with any confidence.

### ETH-Factor Models

Now let's estimate an alternative to the CAPM model that replaces the stock market risk premium with the ETH risk premium. Since the SingularityNET tokens all operate as Ethereum tokens (some have cross-chain compatibility for Binancechain or Cardano), we might expect this to produce a more meaningful relationship.

```{r ethregs}
AGIXfit = lm(AGIX~ETH, data=daily_annrets_xs_complete)
summary(AGIXfit)
SDAOfit = lm(SDAO~ETH, data=daily_annrets_xs_complete)
summary(SDAOfit)
NTXfit = lm(NTX~ETH, data=daily_annrets_xs_complete)
summary(NTXfit)
RJVfit = lm(RJV~ETH, data=daily_annrets_xs_complete)
summary(RJVfit)
HYPCfit = lm(HYPC~ETH, data=daily_annrets_xs_complete)
summary(HYPCfit)
CGVfit = lm(CGV~ETH, data=daily_annrets_xs_complete)
summary(CGVfit)
SOPHfit = lm(SOPH~ETH, data=daily_annrets_xs_complete)
summary(SOPHfit)
BTCfit = lm(BTC~ETH, data=daily_annrets_xs_complete)
summary(BTCfit)
NVDAfit = lm(NVDA~ETH, data=daily_annrets_xs_complete)
summary(NVDAfit)
```
